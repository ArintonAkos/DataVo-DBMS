...
The index layer adds an O(1) GUID fast lane alongside the integer fast lane. Primary-key GUID lookups use a direct \texttt{ConcurrentInt64Int64Map} backed by flat arrays and lock-free reads, which bypasses the allocation overhead and indirection of the C\# standard library dictionary path. The map stores packed 64-bit key halves and row-id payloads in contiguous slots, so the lookup path stays branch-light and cache-friendly. Secondary GUID indexes use key-to-row-id buckets, and the GUID comparer performs a hardware-gated 128-bit equality check by loading both GUID byte sequences into \texttt{Vector128<byte>} registers. The result is a no-boxing, no-string, no-parse path from compiled query parameter to row id.

\subsection{Architectural Guarantee of Snapshot Isolation}
Snapshot isolation is enforced mechanically rather than aspirationally. Isolation comes from copy-on-write row patching: updates are prepared off-path, then the writer publishes a replacement row image instead of mutating the version that readers may already hold. Visibility comes from hardware atomicity: the concurrent maps publish 64-bit reference swaps, so readers observe either the old stable pointer or the new stable pointer, never a torn intermediate state. Together, those two properties define a concrete guarantee. Readers walk immutable snapshots, writers install new versions, and the storage layer never exposes partially applied state to concurrent queries.

This is not a proof of serializability. It is the narrower guarantee the engine needs for the current benchmark matrix: read paths stay isolated from in-flight writes, and committed replacements become visible with atomic pointer publication.
...
\midrule
DataVo (LSM Relaxed) & 50.700 & 0.606 & 204.000 \\
