...
public sealed class DataVoPreparedSelectSingle<T>
{
    ...
    public T? Execute(int value) => ExecuteKey(DataVoCompiledQuery.BuildScalarComparisonKey(value));

    /// <summary>Executes the prepared lookup for a BIGINT predicate value.</summary>
    public T? Execute(long value) => ExecuteKey(DataVoCompiledQuery.BuildScalarComparisonKey(value));
